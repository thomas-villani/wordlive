...
`ops` must be a JSON **array**, not a JSON-encoded string — but a string that
decodes to an array of op objects is accepted anyway, since double-encoding it is
an easy client-side slip. Anything else fails with an error that names the field
and shows the shape wanted, rather than a raw schema-validation dump.
