...
- `wordlive lint [--rule ID/tag] [--exclude ID/tag] [--within ID] [--profile PATH]` — audit publishing-quality defects (dangling headings, multi-page tables with no repeating header, numbered lists Word split into independent runs, direct formatting drifted from the applied style). Severity-ranked findings, each `fixable` one carrying the op `regularize` would run. Then `wordlive regularize [--dry-run] [--rule …] [--within ID] [--profile PATH] [--allow-content]` applies the fixable subset in one atomic-undo step (targeted + idempotent — a second pass is a no-op); returns `{applied, skipped, deferred, findings}`. Formatting fixes apply by default; content-changing fixes (insert a caption/notice, delete a stray paragraph, strip a watermark) are flagged `adds_content` and held in `deferred` unless you pass `--allow-content`. `--profile PATH` loads a JSON house-style config that enables the **policy** rules (`body-justified`, `body-line-spacing`, `table-numeric-right-align`) + their targets, and can override a rule's severity or disable a default. Add `--rule typography` for the text-hygiene cluster — trailing/leading/double spaces, space-before-punctuation, hyphen→en-dash, manual "headings" that aren't styled, tables on mismatched styles (the tag also enables its off-by-default opinion rules); `--rule finalization` for the off-by-default "ready-to-send?" cluster (leftover comments/revisions, track-changes, hidden text, highlight, updatable fields); `--rule academia` for the field-code cluster (broken cross-references, captions numbered with literal text not a SEQ field — both on by default; plus the heuristic, off-by-default `xref-as-literal-text` — a figure/table mentioned by literal number with no REF field); `--rule hyperlinks` for the link cluster (`hyperlink-broken-internal` — a dead internal `\l` jump to a missing bookmark, on by default; plus the off-by-default `hyperlink-bare-for-print` and `hyperlink-display-is-raw-url`, which `--rule print` selects on their own) and `--rule layout` for the §H page-layout/document-level cluster (`page-numbers-present`, `header-footer-consistent`, `draft-watermark-present`, `document-properties-filled` — all off by default, but naming the tag enables them, like `typography`; `--rule notices` selects the profile-driven `confidentiality-notice`/`copyright-notice` on their own); and `--rule structure` (or `--rule headings`) for the §B heading & document-structure cluster (`heading-level-skip`, `empty-heading` on by default; plus the off-by-default `adjacent-headings`, `heading-numbering-manual`, `heading-trailing-period` — which `regularize` strips in place — and `toc-present-and-current`, which naming the tag likewise enables). `wordlive read watermark` reads any text watermark (`{text, sections}` or null); `read format --anchor-id ID` is the underlying probe: effective paragraph/character formatting with each field's style baseline and an `override` flag (the read mirror of `format-paragraph`/`format-run`).
...
- `wordlive insert-block --anchor-id ID --items JSON [--before | --after]` — insert a **contiguous run of styled paragraphs** in one op (a whole bulleted section, a heading + body) in reading order, instead of reverse-ordered `insert` calls. Each item is `"plain text"` or `{text|runs, style?}`; item `text` takes `**bold**`/`*italic*`/`` `code` `` markdown. Reports the block's `range:START-END` — feed it to `list apply --anchor-id range:… --type bulleted` to bullet the section you just inserted.
...
- `wordlive insert-markdown --anchor-id ID --markdown "…"|-` — drop a chunk of **constrained Markdown** as real Word structure: `#`/`##`/`###` headings, `-`/`*` bullets, `1.` numbers, blank-line paragraphs, inline `**bold**`/`*italic*`/`` `code` `` (a code span becomes a monospace run). A documented subset, not CommonMark (no code *fences*/nested lists/tables). Use `--markdown -` for multi-line stdin. **To start a document, lead with this (or `insert-block`/`insert-section`), not `append`** — they reuse a blank doc's lone empty paragraph instead of leaving it stranded above your content.
...
- `wordlive append --text "…" [--inline] [--style "Body Text"]` — add a new final paragraph at the very end of the document (`--inline` continues the last paragraph). The high-level "end of doc" helper; same as `insert --anchor-id end`. Its contract is "a *new* final paragraph", so on a blank document it leaves the original empty `para:1` above your text — lead with `insert-markdown`/`insert-block` instead, or clean up with `delete-paragraph --anchor-id para:1`.
...
- `wordlive replace --find "old" --text "new" [--all | --occurrence N] [--in ID] [--mode fuzzy|literal|regex]` — find + replace. Default `fuzzy` is whitespace/smart-quote tolerant; `--mode literal` is exact; `--mode regex` makes `--find` a Python regex (`--text` may use `\1` backreferences). To edit **inside a table**, scope it to the cell (`--in table:N:R:C`); an unverifiable whole-document match fails (exit 1, `replace_verification`) rather than risk overwriting the wrong cell.
